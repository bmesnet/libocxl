...
static uint64_t mmioLatMode = 0xC000000000000000;
...
static void print_help(char *name)
{
        ...
        printf("\t--mmpp      \tAdd MMIO Ping-pong mode tests to existing BW tests and latency\n");
...
int main(int argc, char *argv[])
{
  ...
  //uint64_t *buffer;
        ...
        if(mmpp && ((tags_ld != 0) || (tags_st == 0))) {
                printf("In MMIO Ping-Pong mode, tags_ld is forced to be 0, and tags_st need to be >0\n");
                tags_ld = 0;
                tags_st = 1;
        }
        ...
        ////////////////////////////////////////////////////////////////////////
        // MMIO Ping Ping mode simple action by the APP
        ////////////////////////////////////////////////////////////////////////
        // AFP will write a counter value to memory cacheline at buffer[0].
        // When this app sees the value change, it sends an MMIO write.
        // When AFP sees any MMIO read or write that is not to the Performance
        // Counter or Latency registers, AFP will send the next write to buffer[0].
        // Keep going for 128 times


        uint16_t i;
        uint64_t data0 = 0, poll_value;
        uint64_t result_buffer[128];
        int iterations = 128;
        if (mmpp)
        {
                data0 = (uint64_t)buffer[0];
                asm volatile("": : :"memory");

                // ENABLE REG for mmpp (setting value and starting test)
                if (verbose)
                        printf("ENABLE_REG = %lx\n", mmioLatMode);
                err = ocxl_mmio_write64(global, AFUEnable_AFP_REGISTER, OCXL_MMIO_LITTLE_ENDIAN, (uint64_t) mmioLatMode);
                if (err != OCXL_OK) {
                        perror("ocxl_mmio_write64:"DEVICE);
                        return err;
                }
                ...
                for(i = 0; i < iterations; ) {
                        // creates a compiler level memory barrier forcing optimizer to not
                        // re-order memory accesses across the barrier.
                        asm volatile("": : :"memory");
                        //while(buffer[0] == data0)
                        // { }
                        poll_value = (uint64_t) buffer[0];
                        if (data0 != poll_value) {
                                err = ocxl_mmio_write64(global, AFUEnable_AFP_REGISTER, OCXL_MMIO_LITTLE_ENDIAN, (uint64_t) mmioLatMode);
                                if (err != OCXL_OK) {
                                        perror("ocxl_mmio_write64:"DEVICE);
                                ...
                                result_buffer[i] = poll_value;
                                data0 = poll_value;
                                i++;
                                //printf("%d ", i);
                        }
                }
                printf("\n%d loops done: MMIO write from APP + DMA write by AFU\n", i);
                double avg_lat = 0.0;
                uint64_t lat_diff, lat_min, lat_max;
                lat_diff = 0;
                lat_min  = 0;
                lat_max = 0;
                ...
                for(i = 1; i < iterations; ) {
                   if (verbose) {
                       printf("%d: Diff: %lld  %0.3f ns\n", i,
                          (long long)( result_buffer[i] - result_buffer[i-1] ),
                          (result_buffer[i] - result_buffer[i-1]) * 5.0);
                   }

                   lat_diff = ( (result_buffer[i] - result_buffer[i-1]) * 5.0);
                   if( i == 1) {
                        lat_min = lat_max = lat_diff;
                   } else if( i > 1) {
                        if(lat_min > lat_diff)
                                lat_min = lat_diff;
                        if(lat_max < lat_diff)
                                lat_max = lat_diff;
                        avg_lat = avg_lat + lat_diff;
                   }
                   i++;
                  }
                  printf ("\n MMPP Average latency: %0.3f ns (min %ld ns, max %ld ns)\n\n",
                             avg_lat/(iterations - 1), lat_min, lat_max);
                  asm volatile("": : :"memory");
        ...
        uint64_t latency_cyc0, latency_cyc, latency_diff, latency_min, latency_max, latency_sum;
        double latency_average;
        uint32_t depth = mmpp?128:2048;
        ...
        // No need to measure the latency in MMIO latency mode
        //There is a 2048-Deep array. Each time you read AFULatency_AFP_REGISTER, the pointer will move to next.
        for (i = 0; i < depth; i++) {
                err = ocxl_mmio_read64(global, AFULatency_AFP_REGISTER, OCXL_MMIO_LITTLE_ENDIAN, &latency_cyc);
                if (err != OCXL_OK) {
                        perror("ocxl_mmio_read64:"DEVICE);
                        return err;
                }
                latency_diff = latency_cyc - latency_cyc0;
                if( i == 1) {
                        latency_min = latency_max = latency_sum = latency_diff;
                } else if( i > 1) {
                        // Lokking for min and max of the latency
                        if(latency_min > latency_diff)
                                latency_min = latency_diff;
                        if(latency_max < latency_diff)
                                latency_max = latency_diff;

                        latency_sum += latency_diff;
                }
                latency_cyc0 = latency_cyc;
        }
        latency_average = (double)latency_sum / (depth - 1);

        printf("Latency ARRAY says: Average %lf (min %ld, max %ld) 200MHz cycles --> %.1lf ns\n", latency_average,
                        latency_min, latency_max, latency_average*5);

        //The second method for FPGA stores:
        //Read buffer if it includes a store (The content is time stamps)
        //I have tested it, the result is as same as above Latency Array Registers. So comment it.
        /* */
        if (tags_st != 0) {
                latency_cyc0 = 0;
                latency_min = 0;
                ...
                latency_sum = 0;
                latency_diff = 0;
                //Only process 1024 write data
                for (i = 0; i < 1024*16; i = i+16)
                {
                        latency_cyc = buffer[i];
                        latency_diff = latency_cyc - latency_cyc0;
                        if( i == 16) {
                                latency_min = latency_max = latency_sum = latency_diff;
                        } else if( i > 16) {
                ...
                latency_average = (double)latency_sum / 1023;

                printf("Stored data in host mem says: Average %lf (min %ld, max %ld) 200MHz cycles --> %.1lf ns\n",
                                latency_average, latency_min, latency_max, latency_average*5);
        }
        /* */
        sleep(1);
